...
FastAIModel is a **retained-memory local inference engine** for Java that wraps the C++ library `llama.cpp` using direct JNI bindings. It is the engine that drives offline execution in the **FastJava Ecosystem**, giving Java developers native LLM (GGUF) and embedding capabilities without keeping heavy external apps (like LM Studio or Ollama) open.
